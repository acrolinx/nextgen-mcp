...
// Type guards
function isBaseToolArgs(args) {
    return (typeof args === 'object' &&
        args !== null &&
        'text' in args &&
        typeof args.text === 'string');
}
function isWorkflowStatusArgs(args) {
    return (typeof args === 'object' &&
        args !== null &&
        'workflow_id' in args &&
        'workflow_type' in args &&
        typeof args.workflow_id === 'string' &&
        ['rewrites', 'checks', 'suggestions'].includes(args.workflow_type));
}
// Input validation
...
async function retryWithBackoff(fn, operationName, maxRetries = MAX_RETRIES, baseDelay = 1000) {
...
async function pollWorkflowCompletion(workflow_id, workflow_type, initialResult) {
